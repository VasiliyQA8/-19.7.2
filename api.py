...
class PetFriends:
    ...
    def get_api_key(self, email: str, password: str) -> json:
        ...
        с уникальным ключём пользователя, найденого по указаным email и password"""
        headers = {
            'email': email,
            'password': password,
        }
    ...
    def add_photo_of_pet(self, auth_key: json, pet_id: str, pet_photo: str):
        '''Метод отправляет (постит) на сервер данные о новом фото указанного
        pet_id питомца и возвращает код статуса запроса и результат в формате json с информацией о животном.'''
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
            })
